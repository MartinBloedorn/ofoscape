...
bool ofxShadertoy::makeShader(std::string const & textBuffer, ofShader & outShader) {
    ...
    if(!currentShader.setupShaderFromSource(GL_FRAGMENT_SHADER, fragprogram)) {
        ...
        if(currentShader.linkProgram()) {
            ...
            currentShader.setUniform4f("iMouse", mousepos.x, mousepos.y, getMousePressed(), 0);
...
void ofxShadertoy::setupShaderUniforms(ofShader const & bufShader, Buffer buffer) {
    ...
    bufShader.setUniform4f("iMouse", mousepos.x, mousepos.y, getMousePressed(), 0);
    ...
    bufShader.setUniformTexture("iChannel0", channels[(size_t)buffer][0], 1 + 4 * instance);
    bufShader.setUniformTexture("iChannel1", channels[(size_t)buffer][1], 2 + 4 * instance);
    bufShader.setUniformTexture("iChannel2", channels[(size_t)buffer][2], 3 + 4 * instance);
    bufShader.setUniformTexture("iChannel3", channels[(size_t)buffer][3], 4 + 4 * instance);
}
...
void ofxShadertoy::applyBuffer(ofShader const & bufShader, Buffer buffer, ofFbo & bufFbo) {
    ...
    channels[(size_t)buffer][3].unbind(4 + 4 * instance);

    glActiveTexture(GL_TEXTURE0);
}

int ofxShadertoy::getMousePressed() const {
    return useMouse ? (ofGetMousePressed() ? 1 : 0) : (virtualMousePressed ? 1 : 0);
}

ofxShadertoy::ofxShadertoy(int instance)
    : globalTime(0)
    , advanceTime(true)
    , camera(NULL)
    , dimensions(640.f, 480.f) 
    , instance(instance)
{
...
void ofxShadertoy::setVirtualMouseState(float x, float y, bool pressed) {
    mousepos.x = x;
    mousepos.y = y;
    virtualMousePressed = pressed;
}
