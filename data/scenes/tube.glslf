...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    rd.xy = rot(iTime*0.25*ofoVar_y)*rd.xy; // Look around, just to show it's a 3D effect.
    rd.xz = rot(iTime*0.125*ofoVar_y)*rd.xz;
    
    // Ray origin.
    //vec3 ro = vec3(0.0, 0.0, iTime);
    vec3 ro = vec3(ofoVar_x, -ofoVar_x, iTime);
    ...
    if(svObjID>.5) objCol *= vec3(1*ofoVar_y, .8*ofoVar_x, .2)*1.35;
    else objCol *= vec3(.1, 2.5*ofoVar_x, 5*ofoVar_y);
